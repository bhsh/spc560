...
/**
 * @file    spc5_edma.h
 * @brief   EDMA helper driver header for RLA components.
 *
 ...
 */
#if (SPC5_HAS_EDMA == TRUE) || defined(__DOXYGEN__)
#define SPC5_EDMA       (*(edma_t *)0xFFF44000UL)
#endif
 ...
 * @api
 */
#define edmaChannelStart(channel) (SPC5_EDMA.SERQR.R = (uint8_t)(channel))
...
#define edmaChannelStop(channel) {                                          \
  SPC5_EDMA.CERQR.R = (uint8_t)(channel);                                   \
  SPC5_EDMA.CDSBR.R = (uint8_t)(channel);                                   \
}
